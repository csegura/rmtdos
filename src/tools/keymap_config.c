    ...
    while (1) {
        ...
        case KEY_DOWN:
            if (g_cur_itm_idx < g_map_entries - 1) {
                ...
                if (g_cur_itm_idx >= g_top_itm_idx + list_height) {
                    g_top_itm_idx++;
                }
            } else {
                // Indicate bottom
        ...
        case KEY_NPAGE: {
            ...
            if (list_height < 1)
                list_height = 1;
            int prev_index = g_cur_itm_idx;
            g_cur_itm_idx += list_height;
            if (g_cur_itm_idx >= g_map_entries) {
                g_cur_itm_idx = g_map_entries - 1;
            }

            if (g_cur_itm_idx >= g_top_itm_idx + list_height) {
                g_top_itm_idx = g_cur_itm_idx - list_height + 1;
            }
            int max_top_index = g_map_entries - list_height;
            if (max_top_index < 0)
                max_top_index = 0;
            if (g_top_itm_idx > max_top_index) {
                g_top_itm_idx = max_top_index;
            }
            if (g_cur_itm_idx < g_top_itm_idx) {
                g_cur_itm_idx = g_top_itm_idx;
            }

            if (g_cur_itm_idx == prev_index && g_cur_itm_idx == g_map_entries - 1) {
                beep();
            }
        } break;
        ...
        default:
            // Optional: Handle other keys or beep
            // beep();
            break;
